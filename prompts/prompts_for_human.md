...
### 프로젝트 기획 업데이트 요청
1. 적용 아키텍쳐 정의 : DDD 아키텍처 적용, MSA 적용, Docker-container 는 MSA 단위로 구성
4. 환경구성요소 설계 : 서버운영환경(AWS EC2), 도커환경(docker-compose, docker-compose.dev, docker-compose.prod), 파이썬가상환경(python3.13 이상, uv.lock, pyproject.toml) 
2. 서버구조 설계 : vercel/page 서버(Next.js), AWS EC2/api서버(fastapi), AWS EC2/DB서버(postrege, Database 테이블 개수는 최소화하여 운영)
5. urls 패턴 설계(page 서버, api 서버 url 라우터에 적용) 
